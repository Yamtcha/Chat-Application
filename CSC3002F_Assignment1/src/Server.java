...
public class Server implements Runnable {
	...
	private void initialiseServer() {
		...
		System.out.println("******************************************\n"
				+ "System Notice - Server Started and waiting for Connections on Port: "
				+ Server.INCOMING_CONNECTION_PORT + ".\n******************************************");
	}
	...
	public synchronized boolean saveUserDetailsToDatabase(String username, String password) {
		...
		System.out.println("------------------------------------------\n" + "System Action - " + username
				+ "'s Details Successfully added to Database." + "\n------------------------------------------");
		return true;
	}
	...
	private class ClientInteractionHandler implements Runnable {
		...
		public ClientInteractionHandler(Socket connectionToClient) {
			...
			try {
				this.oOutputStream = new ObjectOutputStream(
						new BufferedOutputStream(this.connectionToClient.getOutputStream()));
				this.oOutputStream.flush();
				this.oInputStream = new ObjectInputStream(
						new BufferedInputStream(this.connectionToClient.getInputStream()));
			} catch (IOException e) {
				System.out.println(e);
			}
		}
		...
		private void storeMessageinConnectionOutStandingMessages(Message message,
				ClientInteractionHandler clientConnection) {
		...
		public void run() {
			...
			while (!isCorrect) {
				...
				System.out.println("******************************************\n" + "System Notice - Warning: "
						+ input.getSourceName() + " Entered incorrect Client Credentials."
						+ "\n******************************************");
				output = new Message(MessageID.REGISTRATION_RESPONSE, Server.SERVER_NAME, input.getSourceName(),
						isCorrect);
			...
			System.out.println("******************************************\n" + "System Notice - "
					+ input.getSourceName() + " Logged In with correct Client Credentials."
					+ "\n******************************************");
			...
			while (!this.connectionToClient.isClosed()) {
				...
				case IMAGE_TRANSFER_REQUEST: {
					if (checkOnline(input.getDestinationName())) {
						...
						Message imageMessage = new Message(MessageID.IMAGE_TRANSFER_RECEIPT, input.getSourceName(),
								input.getDestinationName(), input.getData());
						this.storeMessageinConnectionOutStandingMessages(imageMessage,
				...
				case CLOSE_CONNECTION: {
					...
					output = new Message(MessageID.CLOSE_CONNECTION, Server.SERVER_NAME, this.clientUsername, "");
					this.sendMessageToClient(output);
					System.out.println(
							"******************************************\n" + "System Notice - " + this.clientUsername
