...
public class Client {
	...
	private void setupConnectToServer(String serverIP) {
		try {
			this.serverConnectionHandler = new ServerInteractionHandler(
					new Socket(serverIP, Client.INCOMING_CONNECTION_PORT));
			System.out.println("*********************************************************************\n"
	...
	public static void main(String args[]) {
		...
		while (!choice.equals("Exit")) {
			...
			case "3": {
				...
				thisClient.setEnteringInput(false);
				// send message to server
				Message output = new Message(MessageID.TEXT_SEND_TO_ALL_REQUEST, thisClient.getUsername(), "All",
						message);
				thisClient.getServerInteractionHandler().sendMessageToServer(output);
			...
			case "4": {
				...
				// send message to server
				Message output = new Message(MessageID.IMAGE_SEND_TO_ALL_REQUEST, thisClient.getUsername(), "All",
						(Object) image);
				thisClient.getServerInteractionHandler().sendMessageToServer(output);
...
>>>>>>> developer
			...
			case "Exit": {
				...
				try {
					// tell the server that the connection is closing
					thisClient.getServerInteractionHandler().sendMessageToServer(
							new Message(MessageID.CLOSE_CONNECTION, thisClient.getUsername(), Client.SERVER_NAME, ""));
				} catch (Exception e) {
					System.out.println(e);
				}
				return;
		...
		public ServerInteractionHandler(Socket connectionToServer) {
			...
			try {
				this.oOutputStream = new ObjectOutputStream(
						new BufferedOutputStream(this.connectionToServer.getOutputStream()));
				this.oOutputStream.flush();
				this.oInputStream = new ObjectInputStream(
						new BufferedInputStream(this.connectionToServer.getInputStream()));
			} catch (IOException e) {
				System.out.println(e);
			}
		}
		...
		public void run() {
			...
			while (!this.connectionToServer.isClosed()) {
				...
				case IMAGE_TRANSFER_RECEIPT: {
					System.out.println("*********************************************************************\n"
							+ "System Notice : " + input.getSourceName() + " sent you an Image Opening in JFrame."
							+ "\n*********************************************************************");
